...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${r"${ctx}"}${urlPrefix}/list">${functionName}列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="${className}" action="${r"${ctx}"}${urlPrefix}/list" method="post" class="breadcrumb form-search">
		...
		<label>备注 ：</label><form:input path="remarks" htmlEscape="false" maxlength="50" class="input-small"/>
		&nbsp;<input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
	</form:form>
	<tags:message content="${r"${message}"}"/>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>创建时间</th><th>备注</th><shiro:hasPermission name="${permissionPrefix}:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		<c:forEach items="${r"${page.list}"}" var="${className}">
			<tr>
				<td><fmt:formatDate value="${"${"+className+".createDate}"}" pattern="yyyy-MM-dd"/></td>
