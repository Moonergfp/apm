	...
	public Page<User> findUser(Page<User> page, User user) {
		...
		if (user.getCompany()!=null && StringUtils.isNotEmpty(user.getCompany().getId())){
			dc.add(Restrictions.or(
					Restrictions.eq("company.id", user.getCompany().getId()), 						 //该公司（机构）
					Restrictions.like("company.parentIds", "%,"+user.getCompany().getId()+",%")      //子公司（机构）
					));
		}else{
			dc.add(Restrictions.or(
					Restrictions.eq("company.id", currentUser.getCompany().getId()), 						 //该公司（机构）
					Restrictions.like("company.parentIds", "%,"+currentUser.getCompany().getId()+",%")      //子公司（机构）
					));
		}
		/*dc.createAlias("office", "office");
		if (user.getOffice()!=null && StringUtils.isNotEmpty(user.getOffice().getId())){
			dc.add(Restrictions.or(																	
					Restrictions.eq("office.id", user.getOffice().getId()),							//部门
					Restrictions.like("office.parentIds", "%,"+user.getOffice().getId()+",%")		//子部门
					));
		}else{
			dc.add(Restrictions.or(																	
					Restrictions.eq("office.id", currentUser.getOffice().getId()),							//部门
					Restrictions.like("office.parentIds", "%,"+currentUser.getOffice().getId()+",%")		//子部门
					));
		}*/
