...
(function($){
	$.pageRuler = function(params){
		...
		var f = {
			box: function(){
				w = $(".content-center").width(), h = $(".content-center").height();
				console.log("width "+w);
			...
			newv: function(t){//创建新的垂直参考线，有效宽度3像素
				...
				var zxxRefLine_v = $('<div class="zxxRefLine_v" id="'+id+'" title="'+t+'" style="'+$(".content-center").height()+'px"></div>')
					x.before(zxxRefLine_v);					
				return $("#"+id);
			},
			newh: function(t){//创建新的垂直参考线，有效宽度3像素
				var id = "zxxRefLineH"+($(".zxxRefLine_h").size()+1);
				var zxxRefLine_h =  $('<div class="zxxRefLine_h" id="'+id+'" title="'+t+'" style="width:'+$(".content-center").width()+'px"></div>')
				x.before(zxxRefLine_h);					
				return $("#"+id);
			},
			dashv: function(){
				$(document).bind("mousemove",function(e){
					...
					if(dragFlag){
						...
						dotv.css("left",e.pageX-$(".content-main-edit").offset().left);
					}
				});
			},
			dashh: function(){
				$(document).bind("mousemove",function(e){
					if(dragFlag){
						//如果可以拖拽
						//垂直虚线的左坐标						
						doth.css("top",e.pageY-$(".content-main-edit").offset().top);
		...
		$(document).mouseup(function(e){
			...
			if(oDrag){
				...
				/*	var v_l = e.pageX;*/
					var v_l = e.pageX-$(".content-main-edit").offset().left;
					if(v_l < rv.width()){
						v_l = -600;					  
					}
				...
				/*	var v_t = e.pageY-$(window).scrollTop();*/
					var v_t = e.pageY-$(".content-main-edit").offset().top;
					if(v_t < rh.height()){
						v_t = 600;	
					}
