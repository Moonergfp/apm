...
<body>
		...
		<div class="div_bottom">
			<ul class="nav nav-tabs">
				...
				<shiro:hasPermission name="sys:office:edit">
					<li>
						<a href="${ctx}/sys/office/form">机构添加</a>
					</li>
				</shiro:hasPermission>
	...
	<div class="container-fluid breadcrumb" style = "padding-top:15px;">
	...
	<div class="breadcrumb select_class">
		<a id="assignButton" href="javascript:" class="btn btn-primary">分配角色</a>
		<!-- <a id="" href="javascript:" class="btn btn-primary" onclick = "showWarn();">分配角色</a>
		<span class = "showWarn" style = "color:red"></span>
		<script type="text/javascript">
			function showWarn(){
				$('.showWarn').empty();
				$('.showWarn').append(" &nbsp;&nbsp这个以后再做！");
			}
		</script> -->
				...
				top.$.jBox.open("iframe:${ctx}/sys/role/usertorole?id=${role.id}", "分配角色",810,$(top.document).height()-240,{
					buttons:{"确定分配":"ok", "清除已选":"clear", "关闭":true}, bottomText:"通过选择部门，然后为列出的人员分配角色。",submit:function(v, h, f){
						...
						if (v=="ok"){
					    	...
					    	var form = $("<form />").attr({method: "POST", action: "${ctx}/sys/role/assignrole?id=${role.id}&idsArr="+idsArr});
					    	$("body").append(form);
							loading('正在提交，请稍等...');
					        form.submit();
					    	return true;
